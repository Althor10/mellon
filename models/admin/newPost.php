...
//            var_dump($errors);
            ...
            if(count($errors) == 0 ){
                ...
                if(move_uploaded_file($thumbnail_tmpLocation, '../../'.$pathNewImgThumb) && move_uploaded_file($firstImage_tmpLocation,'../../'.$pathNewImgFirst) && move_uploaded_file($secondImage_tmpLocation,'../../'.$pathNewImgSecond) && move_uploaded_file($thirdImage_tmpLocation,'../../'.$pathNewImgThird)){
                    ...
                    try {
                        $isInsertedPost = insertPost($title,$subtitle,$text,$pathNewImgThumb);

                        if($isInsertedPost){
                            $isPostAdded = getInsertedPost($pathNewImgThumb);
                            $postId = (int) $isPostAdded[0]->post_id;
                            if($isPostAdded){
                                $isInsertedImgs = insertImgs($postId,$pathNewImgFirst,$pathNewImgSecond,$pathNewImgThird);
                                $isInsertedCat = insertCat($postId,$category);
